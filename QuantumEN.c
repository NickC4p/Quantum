...
void updateQuantum(char *execPath) {
    ...
    if (access(path, F_OK) != -1) {
        char choice;
        printf("Quantum.c found on the Desktop. Do you whant to Update Quantum? After installation restart Quantum and delete the Quantum.c file on the Desktop. (y/n): ");
        scanf(" %c", &choice);
        if (choice == 'y' || choice == 'Y') {
            ...
            int res = system(cmd);
            if (res == 0) {
                printf("Quantum updately correctly!\n Please restart Quantum");
            } else {
        ...
        printf("No Quantum.c file found on the desktop.\n");
    }
}
